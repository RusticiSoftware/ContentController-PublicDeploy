...
We run our QA/Test environment for Content Controller on a t2.medium EC2 instance which has the following spec:

* 4 GB RAM
* 2 vCPU, running on a 2.4 GHz Xeon E5-2676
...
[Please have a look at the Hosting and Infrastructure documentation in our knowledgebase](http://support.scorm.com/hc/en-us/categories/115000007313-Hosting-Infrastructure-and-Servers) - the info there is specific to SCORM Engine, but it is all directly applicable to hosting Content Controller at scale.

[Please have a look at this Architectural Diagram for an overview of a production grade AWS environment](http://media.amazonwebservices.com/architecturecenter/AWS_ac_ra_web_01.pdf), the architecture of which is also _mostly_ applicable to non-AWS environments.
...
You'll need to install Ansible 2.2 or later on a system separate from your Content Controller box.  A Linux or macOS workstation is fine for this.  Don't try to run it on Windows - it won't work.  For production deployments, it's a good idea to have a small system (such as a t2.nano) dedicated to this task.  If you're running at AWS or another cloud provider, you'll see a significant performance boost if you run the Ansible configurations from a system that lives within the Cloud environment, especially if you're controlling a bunch of machines.  

On modern Linux and macOS systems, installing Ansible is usually as easy as running these two commands:
