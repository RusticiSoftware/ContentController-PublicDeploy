...
You'll need to install Ansible 2.0 or later on a system separate from your Content Controller box.  A Linux or macOS workstation is fine for this.  Don't try to run it on Windows - it won't work.  For production deployments, it's a good idea to have a small system dedicated to this task.  If you're running at AWS or another cloud provider, you'll see a significant performance boost if you run the Ansible configurations from a system that lives within the Cloud environment, especially if you're controlling a bunch of machines.  
